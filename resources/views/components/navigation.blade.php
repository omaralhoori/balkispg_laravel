<header class="fixed top-0 w-full z-50 border-b border-primary/10 bg-bg-main/90 backdrop-blur-md">
    <div class="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
        ...
        <div class="flex items-center gap-4">
            <x-language-switcher />
            <a href="{{ route('whatsapp.redirect', ['locale' => app()->getLocale()]) }}" target="_blank" rel="noopener noreferrer" class="bg-gold-gradient hover:brightness-110 rounded-sm text-white px-5 py-2 text-xs font-bold uppercase tracking-wider  hover:text-white transition-colors">{{ __('Contact Us') }}</a>
            <div class="md:hidden text-gray-900">
