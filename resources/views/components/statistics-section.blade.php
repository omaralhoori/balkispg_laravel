...
<!-- Statistics Section -->
<section class="relative py-12 px-4 md:px-10 lg:px-20 overflow-hidden bg-zinc-dark section-border">
    ...
    <div class="max-w-7xl mx-auto flex flex-col gap-6">
        ...
        <div class="flex flex-col items-center text-center gap-1">
            ...
            @endif
            <h2 class="text-4xl md:text-5xl font-bold text-secondary leading-tight font-heading">
                @if($subtitle)
            ...
            @if($description)
                <p class="text-secondary max-w-2xl text-base opacity-80">
                    {{ $description }}
                </p>
            @endif
        </div>
        ...
        <!-- Statistics Flex -->
        <div class="flex flex-wrap justify-center items-stretch gap-6 mt-2">
            @foreach($statistics as $stat)
                <div class=" p-4 rounded-xl flex flex-col items-center text-center gap-2 transition-all duration-300 hover:border-primary/30 hover:scale-105 flex-1 min-w-[200px] max-w-[250px]">
                    @if(isset($stat['icon']))
                    ...
                    <div class="flex flex-col gap-1">
                        <p class="stat-value text-3xl md:text-4xl font-bold text-secondary font-heading leading-none">
                            {{ $stat['value'] ?? '' }}
                        </p>
                        <p class="stat-label text-secondary text-sm font-medium">
                            {{ $stat['label'] ?? '' }}
                        </p>
                    </div>
                </div>
