...
<section class="relative h-screen flex items-center justify-center overflow-hidden">
    ...
    <div class="relative z-10 text-center px-6">
        ...
        <h1 class="text-4xl md:text-7xl font-black text-white mb-6 font-almarai tracking-tight">
            <span class="text-primary">{{ $aboutPage->hero_title_highlight }}</span> {{ str_replace($aboutPage->hero_title_highlight, '', $aboutPage->hero_title) }}
        </h1>
...
<section class="py-24 px-6 relative">
    <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
        <div class="glass-card p-12 rounded-3xl relative overflow-hidden group">
            ...
            <h3 class="text-2xl font-bold text-white mb-6 font-almarai">{{ $aboutPage->vision_title }}</h3>
        ...
        <div class="glass-card p-12 rounded-3xl relative overflow-hidden group">
            ...
            <h3 class="text-2xl font-bold text-white mb-6 font-almarai">{{ $aboutPage->mission_title }}</h3>
...
<section class="py-24 bg-zinc-dark/50">
    <div class="max-w-7xl mx-auto px-6">
        <div class="text-center mb-20">
            ...
            @endif
            <h2 class="text-3xl md:text-5xl font-bold text-white font-almarai">{{ $aboutPage->timeline_title }}</h2>
        </div>
        <div class="relative">
                    ...
                    <div class="relative flex flex-col md:flex-row items-center justify-between">
                        @if(($item['position'] ?? 'left') === 'left')
                            <div class="w-full md:w-[45%] text-right mb-8 md:mb-0">
                                ...
                                <h5 class="text-white text-xl font-bold mb-4 font-almarai">{{ $item['title'] ?? '' }}</h5>
...
<section class="py-24 px-6">
    <div class="max-w-7xl mx-auto">
        <div class="text-center mb-16">
            <h2 class="text-3xl md:text-5xl font-bold text-white font-almarai mb-6 text-center">{{ $aboutPage->values_title }}</h2>
            <div class="h-1 w-20 bg-primary mx-auto"></div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            @foreach($aboutPage->core_values as $value)
                <div class="bg-zinc-dark p-10 border border-white/5 rounded-2xl hover:border-primary/30 transition-all duration-300 text-center">
                    <span class="material-symbols-outlined text-primary text-5xl mb-6">{{ $value['icon'] ?? '' }}</span>
                    <h4 class="text-white text-xl font-bold mb-4 font-almarai">{{ $value['title'] ?? '' }}</h4>
...
<section class="py-24 bg-zinc-dark/30">
    <div class="max-w-7xl mx-auto px-6">
        <div class="text-center mb-16">
            ...
            @endif
            <h2 class="text-3xl md:text-5xl font-bold text-white font-almarai">{{ $aboutPage->team_title }}</h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
            @foreach($aboutPage->team_members as $member)
                <div class="group relative overflow-hidden rounded-2xl">
                    ...
                    @endif
                    <div class="absolute inset-0 bg-gradient-to-t from-bg-main via-transparent to-transparent opacity-90"></div>
...
<section class="max-w-4xl mx-auto px-6 text-center mb-24">
    ...
    @endif
    <h2 class="text-4xl md:text-5xl font-black text-white mb-8 font-almarai leading-tight">{{ $aboutPage->commitment_title }} <span class="text-primary">{{ $aboutPage->commitment_title_highlight }}</span></h2>
...
<section class="max-w-7xl mx-auto px-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        @foreach($aboutPage->commitment_sections as $section)
            <div class="glass-card p-10 rounded-2xl relative overflow-hidden group hover:border-primary/30 transition-all duration-500">
                ...
                <div class="flex items-center gap-5 mb-8">
                    ...
                    <h3 class="text-2xl font-bold text-white font-almarai">{{ $section['title'] ?? '' }}</h3>
                </div>
...
<section class="max-w-7xl mx-auto px-6 pb-6 mt-24">
    <div class="border-2 border-primary/40 bg-primary/5 rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center gap-10">
...
<section class="max-w-4xl mx-auto px-6 mt-32 pb-6 text-center">
    ...
    @if($aboutPage->contact_question)
        <h5 class="text-white text-xl font-bold font-almarai mb-6">{{ $aboutPage->contact_question }}</h5>
    @endif
